...
### OSM Топо
![OpenFreeMap](/assets/images/ui-maps-and-layers/osmtopo.png)

Охват: **весь мир**

Варианты: **День/Ночь/Схема**

Карта на основе открытого бесплатного OpenStreetMap, дополненная данными космических аппаратов. По сравнению с «обычными» OSM картами состоит из трёх частей:
- Нижняя часть - простая поверхность суши, с полями, болотами, песком и прочим;
- Средняя часть не имеет отношения к OSM и содержит информацию о затоплениях по данным космических аппаратов, данные о рельефе (из источников ArcticDEM / Copernicus DEM / FABDEM), световая засветка на карте «Ночь» и данные о высоте деревьев на карте «День»;
- Верхняя часть - это снова OSM с его дорогами, водоёмами, домами, надписями.
...
## Слои
### Рельеф
![Рельеф](/assets/images/ui-maps-and-layers/relief.png)

Варианты: **Затенение/Контуры**

### Покрытие сетей мобильных операторов
![Покрытие сетей](/assets/images/ui-maps-and-layers/mobile-networks.png)

Отображает базовые станции мобильных операторов и ожидаемое качество связи.

Варианты: для РФ **Мегафон/МТС/Т2/Билайн**, для РБ **МТС Беларусь/A1/Life**, для Малайзии **Yes/Maxis**

### Лесные кварталы
![Лесные кварталы](/assets/images/ui-maps-and-layers/quarters.png)
...
В действительности состоит из нескольких гибридных слоев, между которыми можно переключаться: Google+OSM, Яндекс+OSM, OSM, Яндекс, Google, Морские навигационные знаки, ГГЦ Гибрид, Росреестр Гибрид. Гибридный слой на данных OpenStreetMap рассчитывается нашей системой. Цвета подобраны так, чтобы сочетаться со слоем Яндекса и создавать видимость того, что это - один слой. Могут накладываться и на темные, и на светлые подложки из-за подобранного набора цветов и условных обозначений.
...
### Архив поисков/Треки/Прочие треки
![Треки поисков](/assets/images/ui-maps-and-layers/sar-tracks.png)

Горячая клавиша: **A**

На этом слое можно посмотреть треки поисков, полученные из разных источников. При выборе слоя на карте отображаются те треки, которые выбраны в наборе слоев (Экстремум/Лиза Алерт/Отклик/Треки ТГ бота), остальные треки можно посмотреть в разделе "Прочие треки". Если в настройках слоев отряд не выбран, тогда этот слой будет называться "Архив поисков".
